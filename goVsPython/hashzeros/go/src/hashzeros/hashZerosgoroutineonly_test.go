...
func TestGetBestZerosGoroutine(t *testing.T) {
	var test = struct {
		text          string
		nonceLimit    int
		oprationLimit int
	...
	got := GetBestZerosGoroutine(test.text, test.nonceLimit, test.oprationLimit)
...
//!+bench
func BenchmarkGetBestZerosGoroutine(b *testing.B) {
	GetBestZerosGoroutine("test", b.N, 1000)
}
